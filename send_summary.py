    ...
    try:
        ...
        payload = {
            ...
            "messages": [{
                "role": "user",
                "content": (
                    "Ты — финансовый аналитик. Составь краткую утреннюю рыночную сводку "
                    "на русском в формате для Telegram (HTML: <b>, <a href>). Объём — средний: "
                    "по каждому блоку 1 короткое пояснение, по событиям — что они значат для рынка "
                    "и волатильности. Не выдумывай цифры — используй только данные ниже. "
                    f"Дата: {now.strftime('%A, %d.%m.%Y')}.\n\n"
                    f"Данные (JSON):\n{json.dumps(data, ensure_ascii=False, indent=2)}\n\n"
                    "Структура: заголовок с датой; Погода (Одесса); Курс валют (НБУ); "
                    "Крипторынок; Форекс EUR/USD; Главные события дня со ссылками; "
                    "короткий вывод по волатильности; строка Источники с ссылками на "
